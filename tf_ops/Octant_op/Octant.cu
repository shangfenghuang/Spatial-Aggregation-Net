...
__global__ void cube_select(int b, int n, int n_new,float radius, const float* xyz, const float* xyz_new, int* idx_out) {
    int batch_idx = blockIdx.x;
    xyz += batch_idx * n * 3;
    xyz_new +=batch_idx * n_new * 3;
    ...
    for(int i = threadIdx.x; i < n;i += blockDim.x) {
        ...
        for(int j = 0;j < n_new;j ++) {
            if(i == j) continue;
            float tx = xyz_new[j * 3];
            float ty = xyz_new[j * 3 + 1];
            float tz = xyz_new[j * 3 + 2];
...
__global__ void cube_select_two(int b, int n,int n_new,float radius, const float* xyz,const float* xyz_new, int* idx_out) {
    int batch_idx = blockIdx.x;
    xyz += batch_idx * n * 3;
    xyz_new +=batch_idx * n_new * 3;
...
__global__ void cube_select_four(int b, int n,int n_new,float radius, const float* xyz,const float* xyz_new, int* idx_out) {
    int batch_idx = blockIdx.x;
    xyz += batch_idx * n * 3;
    xyz_new +=batch_idx * n_new * 3;
...
__global__ void cube_select_eight(int b, int n,int n_new,float radius, const float* xyz,const float* xyz_new, int* idx_out) {
    int batch_idx = blockIdx.x;
    xyz += batch_idx * n * 3;
    xyz_new +=batch_idx * n_new * 3;
